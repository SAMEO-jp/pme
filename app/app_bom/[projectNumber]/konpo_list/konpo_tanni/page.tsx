...
import {
  ...
  TableHeader,
  TableRow,
} from "@/app/app_bom/src/ui/table";
import { Button } from "@/app/app_bom/src/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/app/app_bom/src/ui/card";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/app/app_bom/src/ui/dialog";
import { Input } from "@/app/app_bom/src/ui/input";
import { Label } from "@/app/app_bom/src/ui/label";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/app/app_bom/src/ui/select";
import { toast } from "sonner";
import { Checkbox } from "@/app/app_bom/src/ui/checkbox";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/app/app_bom/src/ui/tabs";
...
export default function KonpoUnitPage() {
  ...
  const fetchKonpoUnits = async () => {
    try {
      ...
      const response = await fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/unit`);
      if (!response.ok) {
  ...
  const fetchUnlinkedParts = async () => {
    try {
      const response = await fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/parts/unlinked`);
      if (!response.ok) {
  ...
  const handleDeleteUnit = async (unitId: string) => {
    ...
    try {
      const response = await fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/unit?id=${unitId}`, {
        method: 'DELETE',
      });
  ...
  const handleListMake = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/unit/list`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleBulkCreate = async () => {
    ...
    try {
      ...
      const promises = unlinkedParts.map(part => {
        ...
        return fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/unit`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleUpdateWeights = async () => {
    try {
      const response = await fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/unit/part_tanni_weight`, {
        method: 'POST',
      });
  ...
  const handleRefreshParts = async () => {
    try {
      const response = await fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/unit/refresh_parts`, {
        method: 'POST',
      });
      if (!response.ok) {
  ...
  const handleCreateKonpoList = async () => {
    try {
      ...
      const promises = konpoLists.map(async (list) => {
        const response = await fetch(`/api/app_bom/${projectNumber}/konpo_list/konpo_tanni/list`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
