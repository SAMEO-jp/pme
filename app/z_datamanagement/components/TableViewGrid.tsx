...
const TableViewGrid: React.FC<TableViewGridProps> = ({
  ...
  const toggleRowSelection = (row: any) => {
    setSelectedRows((prev: any[]) => {
      const pkColumns = columns.filter(col => col.isKey).map(col => col.key);
      const isSelected = prev.some((r: any) => {
        // 主キーが存在する場合は主キーで比較
        if (pkColumns.length > 0) {
          return pkColumns.every((pk: string) => r[pk] === row[pk]);
        }
        // 主キーがない場合は全フィールドで比較
  ...
  const toggleSelectAll = () => {
    ...
    setSelectedRows(newSelectAll ? [...currentData] : []);
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden h-[1000px] flex flex-col">
      <div className="py-2 px-3 flex items-center gap-x-2 border-b bg-gray-50">
        {/* テーブル名 */}
        <div className="font-bold text-lg flex-shrink-0">{headerTitle}</div>
        {/* アイコン群（headerActions） */}
        <div className="flex gap-x-2 items-center">{headerActions}</div>
        {/* 列編集ボタン */}
        <ActionButton 
          variant="secondary" 
          size="small"
        ...
        <div className="relative ml-auto w-56">
          <input
            type="text"
            placeholder="検索..."
            className="pl-8 pr-4 py-1 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm w-full"
            value={searchTerm}
            onChange={(e) => {
      ...
      <div className="overflow-x-auto flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 w-[1620px]">
