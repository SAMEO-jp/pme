...
export function getWeekDataFromStorage(
  ...
  try {
    const data = localStorage.getItem(getWeekStorageKey(year, week));
...
export function setWeekDataChanged(
  ...
  try {
    localStorage.setItem(
      getWeekChangedFlagKey(year, week),
      changed ? "true" : "false"
    );
  } catch (error) {
...
export function hasWeekDataChanged(
  ...
  try {
    return localStorage.getItem(getWeekChangedFlagKey(year, week)) === "true";
  } catch (error) {
    console.error(
...
export function clearWeekData(year: number, week: number) {
  try {
    localStorage.removeItem(getWeekStorageKey(year, week));
    localStorage.removeItem(getWeekChangedFlagKey(year, week));
  } catch (error) {
