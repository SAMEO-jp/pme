...
export async function getWorkTimes(year: number, week: number): Promise<WorkTime[]> {
  try {
    const response = await fetch(`/api/kintai/week/${year}/${week}`)
    if (!response.ok) {
      throw new Error(`勤務時間データの取得に失敗しました: ${response.status}`)
    }
    const data = await response.json()
    return data.data
  } catch (error) {
    throw new Error(`勤務時間データの取得エラー: ${error instanceof Error ? error.message : String(error)}`)
  }
}

export async function updateWorkTime(workTime: WorkTime): Promise<WorkTime> {
  try {
    const response = await fetch(`/api/kintai/${workTime.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      throw new Error(`勤務時間データの更新に失敗しました: ${response.status}`)
    }

    const data = await response.json()
    return data.data
  } catch (error) {
    throw new Error(`勤務時間データの更新エラー: ${error instanceof Error ? error.message : String(error)}`)
  }
}

export async function getWeekWorkTimes(year: number, week: number): Promise<WorkTime[]> {
  try {
    const response = await fetch(`/api/kintai/week/${year}/${week}`)
    if (!response.ok) {
      throw new Error(`週の勤務時間データの取得に失敗しました: ${response.status}`)
    }
    const data = await response.json()
    return data.data
  } catch (error) {
    throw new Error(`週の勤務時間データの取得エラー: ${error instanceof Error ? error.message : String(error)}`)
  }
}

export async function updateWeekWorkTimes(year: number, week: number, workTimes: WorkTime[]): Promise<WorkTime[]> {
  try {
    const response = await fetch(`/api/kintai/week/${year}/${week}`, {
      method: 'PUT',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`週の勤務時間データの更新に失敗しました: ${response.status}`)
    }

    const data = await response.json()
    return data.data
  } catch (error) {
    throw new Error(`週の勤務時間データの更新エラー: ${error instanceof Error ? error.message : String(error)}`)
  }
} 